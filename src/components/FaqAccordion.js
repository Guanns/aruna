...
export default function FaqAccordion() {
    ...
    return (_jsxs("div", { className: "w-full max-w-3xl mx-auto py-16 px-6", children: [_jsx("h2", { className: "text-3xl md:text-4xl font-bold text-center mb-4 text-[#6B4F4F]", children: "Sering Ditanyakan" }), _jsx("p", { className: "text-center text-[#6B4F4F]/60 mb-12", children: "Kami rangkum jawaban untuk pertanyaan populer Kamu!" }), _jsx("div", { className: "space-y-4", children: faqs.map((faq, index) => {
                    const isOpen = openIndex === index;
                    return (_jsxs("div", { className: `bg-white rounded-2xl overflow-hidden transition-all duration-300 border border-transparent
                                ${isOpen ? 'shadow-lg shadow-[#6B4F4F]/5 border-[#6B4F4F]/5' : 'shadow-sm hover:shadow-md'}
                            `, children: [_jsxs("button", { onClick: () => toggleFaq(index), className: "w-full flex justify-between items-center text-left p-6 md:p-8", children: [_jsx("span", { className: `font-bold text-lg md:text-xl transition-colors ${isOpen ? 'text-[#c43c27]' : 'text-[#6B4F4F]'}`, children: faq.question }), _jsx("span", { className: `ml-4 p-2 rounded-full transition-all duration-300 ${isOpen ? 'bg-[#c43c27] text-white rotate-180' : 'bg-stone-100 text-[#6B4F4F]'}`, children: isOpen ? _jsx(MinusIcon, { className: "w-5 h-5" }) : _jsx(PlusIcon, { className: "w-5 h-5" }) })] }), _jsx("div", { className: `grid transition-all duration-500 ease-in-out ${isOpen ? 'grid-rows-[1fr] opacity-100' : 'grid-rows-[0fr] opacity-0'}`, children: _jsx("div", { className: "overflow-hidden", children: _jsx("div", { className: "p-6 md:p-8 pt-0 text-[#6B4F4F]/70 leading-relaxed", children: faq.answer }) }) })] }, index));
                }) })] }));
}
