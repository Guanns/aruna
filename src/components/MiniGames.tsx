...
export function StoneStackingGame() {
    ...
    const [fallingStone, setFallingStone] = useState<{
        width: number;
        x: number;
        color: string;
        startBottom: number;
    ...
    const animate = useCallback(() => {
        if (!isPlaying || gameOver || isFalling) return;
    ...
    const placeStone = (e: React.MouseEvent<HTMLDivElement>) => {
        ...
        if (isFalling) return; // Prevent clicking while stone is falling
    ...
    return (
        ...
        >
            ...
            >
                {/* Falling Stone animation element */}
                {isFalling && fallingStone && (
                    <div
                        className={`${fallingStone.color} shadow-lg rounded-sm border-b-2 border-black/10 absolute ${fallingStone.isMiss ? 'animate-stone-miss-drop' : 'animate-stone-drop'}`}
                        style={{
                ...
                {stack.map((stone, i) => (
                    <div
                        key={stone.id}
                        className={`${stone.color} shadow-lg rounded-sm border-b-2 border-black/10 absolute`}
                        style={{
