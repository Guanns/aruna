...
export default function LawPage() {
    ...
    return (
        <div className="w-full min-h-screen bg-[#FFFBF5] text-[#6B4F4F] relative overflow-hidden font-sans pb-24">
            ...
            <div className="fixed inset-0 pointer-events-none z-0 overflow-hidden">
                 <div className="absolute top-[-10%] left-[20%] w-[600px] h-[600px] bg-indigo-150/15 rounded-full blur-[120px] animate-pulse" style={{ animationDuration: '9s' }}></div>
                 <div className="absolute bottom-[10%] right-[-10%] w-[500px] h-[500px] bg-teal-150/15 rounded-full blur-[100px]" style={{ animationDuration: '11s' }}></div>
            ...
            <div className="max-w-5xl mx-auto px-6 pt-32 relative z-10">
                ...
                <header className="text-center mb-16 max-w-2xl mx-auto">
                    <div className="flex justify-center mb-6">
                        <div className="w-16 h-16 bg-indigo-50 text-indigo-600 rounded-2xl flex items-center justify-center shadow-md border border-indigo-100/50">
                            <ScaleIcon className="w-8 h-8" />
                        </div>
                    </div>
                    <h1 className="text-4xl md:text-5xl font-extrabold mb-4 tracking-tight leading-tight">
                        Dasar Hukum <span className="font-serif italic text-indigo-600">Aruna</span>
                    </h1>
                    <p className="text-sm md:text-base opacity-70 leading-relaxed font-light">
                        Landasan regulasi konstitusional di Indonesia yang melindungi korban kekerasan seksual, fisik, psikis, maupun verbal di berbagai ranah kehidupan.
                    </p>
                </header>
                    ...
                    {LAW_CATEGORIES.map(category => {
                        ...
                        return (
                            <button
                                ...
                                `}
                            >
                                <div className={`p-2.5 rounded-xl transition-colors ${activeTab === category.id ? 'bg-indigo-50/80 text-indigo-600' : 'bg-stone-100/80 text-stone-400'}`}>
                                    <IconComponent className="w-5 h-5" />
                                </div>
                                <span className="font-bold text-sm tracking-tight text-gray-800">{category.shortTitle}</span>
                            </button>
                        );
                    })}
                </div>

                {/* Active Category Meta Header (Clean and Clear) */}
                <div className="max-w-5xl mx-auto mb-10 p-6 bg-white/60 backdrop-blur-md rounded-[2.5rem] border border-white flex flex-col md:flex-row justify-between items-start md:items-center gap-4 shadow-sm">
                    <div>
                        <h2 className="text-xl md:text-2xl font-bold text-gray-800 tracking-tight">{activeCategory.title}</h2>
                        <p className="text-xs font-semibold text-indigo-600 mt-1">{activeCategory.subtitle}</p>
                    </div>
                </div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
                    ...
                    <div className="lg:col-span-7 space-y-4">
                        <h4 className="font-bold text-gray-500 text-xs uppercase tracking-wider mb-4 px-2">Daftar Regulasi</h4>
                            ...
                            return (
                                <div key={idx} className="space-y-3">
                                    <div
                                        ...
                                        className={`p-6 rounded-[2rem] border transition-all duration-300 cursor-pointer group flex justify-between items-center
                                            ${isSelected 
                                    ...
                                    {/* Responsive Mobile Accordion Details */}
                                    {isSelected && (
                                        <div className="block lg:hidden bg-gradient-to-br from-white to-indigo-50/20 rounded-[2rem] p-6 shadow-inner border border-indigo-50/50 animate-fade-in space-y-4 mx-2">
                    ...
                    {/* Detail Sidebar / Panel (Desktop only) */}
                    <div className="hidden lg:block lg:col-span-5">
                        <div className="flex items-center gap-2 mb-4 px-2">
                            <ShieldCheckIcon className="w-5 h-5 text-[#6B4F4F]/60" />
                            <h4 className="font-bold text-gray-700 text-sm uppercase tracking-wider">Penjelasan Detail</h4>
                        </div>
                        <div className="bg-gradient-to-br from-white to-indigo-50/30 rounded-[2.5rem] p-8 shadow-md border border-white/80 min-h-[300px] flex flex-col justify-between relative overflow-hidden">
                            <div className="absolute right-0 top-0 w-24 h-24 bg-indigo-100/10 rounded-full blur-xl pointer-events-none"></div>
                            {selectedLaw ? (
                                <div className="space-y-6 animate-fade-in">
                                    <div>
                                        <h3 className="text-2xl font-bold text-gray-800 tracking-tight">{selectedLaw.article}</h3>
                                        <p className="text-sm font-semibold text-indigo-600 mt-1">{selectedLaw.type}</p>
                                    </div>
                                    <div className="h-px bg-indigo-100/60"></div>
                                    <p className="text-sm md:text-base text-stone-600 font-light leading-relaxed">
                                        {selectedLaw.detail}
                                    </p>
                                </div>
                            ) : (
                                <div className="flex flex-col items-center justify-center py-12 text-center my-auto opacity-60">
                                    <SparklesIcon className="w-10 h-10 text-indigo-400 mb-4 animate-bounce" />
                                    <p className="text-sm font-medium text-stone-400">Pilih salah satu pasal di samping untuk melihat rincian dasar hukum.</p>
                                </div>
                            )}
                            <div className="mt-8 flex items-center gap-2 text-[10px] font-extrabold text-stone-400 uppercase tracking-widest pt-4 border-t border-indigo-100/30">
                                <span>Sumber Kontitusi: NKRI</span>
                            </div>
                        </div>
                    </div>
